...
// 2. 返回时拷贝构造临时对象（再分配100万个int）,这是因为cpp的局部变量会在return前销毁
...
*转发/万能引用（Forwarding Reference/Universal Reference）*：
```cpp
template<typename T>
...
} // typename 避免编译器混淆type是类型还是种类，这里用typename声明type是类型
...
*按引用传递（重要！）*：
```cpp
// 避免拷贝，同时保证不修改原对象,还可以传递临时对象，这是源于没有右值引用时c++98的特性
...
#table(
  ...
  [本质], [C++ 运算符，可以重载], [C/C++ 标准库函数，不可重载],
  [内存块大小], [编译器自动计算，无需指定], [需要显式指定大小],
  [返回值类型], [对象类型的指针], [`void*`，需要类型转换],
  [构造/析构函数], [会调用构造函数和析构函数], [不会调用构造函数和析构函数],
  [内存分配位置], [自由存储区（默认在堆上，但可通过重载从任意位置分配，如栈、静态区、内存池等）], [堆（固定从堆分配，无法改变）],
  [类型安全], [有类型安全检查], [无类型安全检查],
)
...
运算符优先级决定了表达式中运算的顺序，是C++中容易出错的重要知识点。
...
#table(
  ...
  [2], [`()`  `[]`  `.`  `->`  `++`  `--` (后置)], [左到右], [成员访问、下标、函数调用、后置递增/递减],
  
  [3], [`++`  `--` (前置)  `+`  `-` (一元)  `!`  `~`  `*` (解引用)  `&` (取地址)  `sizeof`  `new`  `delete`  类型转换], [右到左], [一元运算符],
...
Node* ptr = new Node{42, nullptr};

// 错误理解
*ptr.value;     // 错误！等价于 *(ptr.value)，编译失败
                // . 的优先级高于 *

// 正确写法
(*ptr).value;   // 先解引用，再访问成员
ptr->value;     // 推荐：箭头运算符

// 陷阱2：函数指针声明
int* func();        // func是函数，返回int*（指针函数）
int (*func)();      // func是指针，指向返回int的函数（函数指针）
                    // () 优先级高于 *，需要括号改变优先级

// 陷阱3：数组指针 vs 指针数组
int* arr[10];       // arr是数组，包含10个int*（指针数组）
int (*arr)[10];     // arr是指针，指向包含10个int的数组（数组指针）
```
...
// 错误
if (flags & 0x04 == 0x04) {  // 等价于: flags & (0x04 == 0x04)
    // == 优先级高于 &
    // 实际计算: flags & 1
}

// 正确
if ((flags & 0x04) == 0x04) {  // 先位运算，再比较
    std::cout << "Flag is set\n";
}

// 更多例子
int a = 5, b = 3;
if (a & b == 0) {           // 错误！等价于 a & (b == 0)
if ((a & b) == 0) {         // 正确：先位与，再比较
```
...
// 前置 vs 后置 vs 解引用
*p++;       // 等价于: *(p++)，后置++优先级高，返回旧指针值并解引用，然后p递增
*++p;       // 等价于: *(++p)，先递增p，再解引用
++*p;       // 等价于: ++(*p)，解引用后递增值
(*p)++;     // 先解引用，再后置递增
...
// 实际效果示例
p = arr;
int val1 = *p++;    // val1 = 1, p指向arr[1]
p = arr;
int val2 = *++p;    // val2 = 2, p指向arr[1]
p = arr;
int val3 = ++*p;    // val3 = 2, arr[0]变成2, p不变
p = arr;
int val4 = (*p)++;  // val4 = 1, arr[0]递增, p不变
```
...
int result = a ? b : c ? 10 : 20;
// 等价于: a ? b : (c ? 10 : 20)
// 不是: (a ? b : c) ? 10 : 20

// 建议：复杂情况使用括号
int result = a ? b : (c ? 10 : 20);  // 清晰
```
...
// 连续赋值返回左值引用
(a = b) = c;    // 合法！a = b返回a的引用，然后a = c
```

*7. 逗号运算符（优先级最低）*

```cpp
// 逗号运算符：从左到右计算，返回最右边的值
int a = (1, 2, 3);      // a = 3
int b = 1, 2, 3;        // 错误！这是声明，不是逗号运算符

// 逗号运算符 vs 函数参数分隔符
func(a, b);             // 函数参数，不是逗号运算符
func((a, b));           // 逗号运算符，传入b的值
...
```cpp
// 类型转换优先级较高（优先级3）
int* p = (int*)malloc(sizeof(int));  // (int*) 优先级高

// 容易混淆的情况
int arr[10];
sizeof arr / sizeof *arr;    // 正确：sizeof优先级高于 /
                             // 等价于: (sizeof arr) / (sizeof(*arr))

// C++风格转换
auto p = static_cast<int*>(ptr);     // 明确的类型转换
```
...
// 4. 宏定义中（防止优先级问题）
#define SQUARE(x) ((x) * (x))   // 每个x都加括号
#define MAX(a, b) ((a) > (b) ? (a) : (b))  // 整体和每个参数都加括号
```
...
// ✓ 推荐：不确定优先级时，使用括号
if ((flags & MASK) != 0) { }
int value = (base + offset) * scale;

// ✓ 推荐：复杂表达式拆分成多行
int result = compute_base()
           + compute_offset() * scale
           + adjustment;

// ✗ 避免：过度依赖优先级的复杂表达式
int x = a & b | c ^ d && e || f ? g : h;  // 难以理解

// ✓ 推荐：拆分并使用中间变量
int and_result = a && e;
int bit_ops = (a & b) | (c ^ d);
int x = and_result || f ? g : h;
```
...
int result = COMPLEX_MACRO(1, 2);
// 使用编译器选项 -E 查看展开结果

// 使用静态断言检查优先级理解
static_assert(sizeof(int) * 8 == 32, "Check precedence");
// 如果理解错误，编译时会报错
```

*总结*：
1. 指针和成员访问相关运算符容易混淆，注意 `*` 和 `.` 的优先级
2. 位运算优先级低于关系运算符，比较时需要括号
3. `&&` 优先级高于 `||`，但建议显式使用括号
4. 不确定时使用括号，提高可读性永远是正确的选择
5. 复杂表达式拆分成多个简单表达式，提高代码可维护性

= 函数指针
函数指针就是一个指向函数的指针变量，它存储了函数在内存中的地址。
...
MemberFunc ptr = &Math::add;  // 使用&取地址
...
*在 C++ 中，struct 和 class 几乎完全相同*，唯一的本质区别是：
1. 成员的默认访问权限不同
2. 继承的默认方式不同

== 相同点

struct 和 class 在以下方面完全相同：

1. 都可以有构造函数、析构函数、拷贝构造、移动构造等
2. 都可以有成员函数（包括虚函数、纯虚函数）和成员变量（包括静态成员）
3. 都可以继承和被继承，支持多重继承
4. 都可以实现多态（虚函数机制）
5. 都可以定义友元（friend）
6. 都可以有模板参数
...
```cpp
// struct 可以做 class 能做的一切
struct MyStruct {
private:  // 可以显式指定 private
    int data;
    
public:
    MyStruct() : data(0) {}
    virtual ~MyStruct() = default;  // 虚析构函数
    virtual void display() const { }  // 虚函数
    
    static int count;  // 静态成员
    
    friend class Helper;  // 友元
};

// class 也可以做 struct 能做的一切
class MyClass {
public:  // 可以显式指定 public
    int x, y;  // 简单数据成员
...
#table(
  ...
  [成员默认访问权限], [public（公有）], [private（私有）],
  
  [继承默认方式], [public（公有继承）], [private（私有继承）],
  
  [设计意图], [C 兼容、表示简单数据结构], [面向对象设计、强调封装],
  
  [典型使用场景], 
  [POD 类型、数据传输对象(DTO)、\ 与 C 代码交互、配置结构], 
  [需要封装实现细节、\ 复杂的继承体系、\ 状态机、业务逻辑类],
  
  [命名习惯], [通常首字母大写或全小写], [通常首字母大写],
)
...
// 使用示例
S s;
s.x = 10;    // ✓ 合法，struct 成员默认 public

C c;
// c.x = 10; // ✗ 错误，class 成员默认 private
```
...
// 等价于: struct DerivedS : public Base
// Base::value 在 DerivedS 中仍是 public

// class 默认私有继承  
class DerivedC : Base { };   
// 等价于: class DerivedC : private Base
// Base::value 在 DerivedC 中变为 private

// 外部访问
DerivedS ds;
ds.value = 10;  // ✓ 合法，public 继承保持 public

DerivedC dc;
// dc.value = 20; // ✗ 错误，private 继承使成员变为 private
```
...
4. *最佳实践*：
- ✓ 使用 `struct`：简单数据聚合、所有成员通常都是 public、与 C 代码交互
- ✓ 使用 `class`：需要封装、有私有成员、复杂的业务逻辑、继承体系
- ✓ *无论使用哪个，建议显式指定访问权限和继承方式*，提高代码可读性和可维护性
- ✓ 团队应统一编码规范，保持一致性

5. *常见误区*：
```cpp
// 误区1：认为 struct 不能有成员函数
struct Vector {
    double x, y;
    double length() const { return sqrt(x*x + y*y); }  // ✓ 完全合法
};

// 误区2：认为 struct 不能实现多态
struct Animal {
    virtual void speak() { }  // ✓ 可以有虚函数
...
= C++中的nullptr与NULL的区别
nullptr 为 C++11引入的关键字，表示一种特殊的空指针类型，具体为std::nullptr_t类型，它是一个空指针常量，可以隐式转换为任意的指针类型，但不能转换为整数类型。
...
int main() {
    ...
    int* p1 = NULL;      // ✅ 可以转换为指针（隐式）
    int n1 = NULL;       // ✅ 也可以转换为整数（隐式）- 这就是问题所在！
    
    // process(NULL);    // ❌ 编译错误：歧义！不知道调用哪个
    
    // 2. nullptr 的优势
    int* p2 = nullptr;   // ✅ 隐式转换为指针
    // int n2 = nullptr; // ❌ 编译错误：不能转换为整数
    
    process(nullptr);    // ✅ 明确调用 process(int*)
    
    // 3. 显式转换 vs 隐式转换
    double d = 3.14;
    int i1 = static_cast<int>(d);  // 显式转换
    int i2 = d;                     // 隐式转换（编译器自动）
...
```cpp
// 不是函数，是编译期操作符，返回类型是 size_t
sizeof(int);              // 编译时就确定值，不在运行时计算
...
// 证明是编译期常量
constexpr size_t s = sizeof(int);  // ✓ 可用于编译期常量
int arr[sizeof(double)];           // ✓ 可用作数组大小
```

*2. 计算规则*

- 基本类型：返回该类型占用的字节数
- 指针：返回指针本身的大小（32位系统4字节，64位系统8字节），与指向的类型无关
- 数组：返回整个数组的大小（元素数量 × 单个元素大小）
- 类/结构体：返回对象的实际大小（包括内存对齐填充）
...
*对齐规则*：
1. 结构体起始地址是最宽成员大小的整数倍
2. 每个成员的偏移量是该成员大小的整数倍
3. 结构体总大小是最宽成员大小的整数倍

```cpp
struct Example {
    char c;   // 1 字节，偏移量 0
    // [3 字节填充] - 为了让 int 对齐到 4 的倍数
    int i;    // 4 字节，偏移量 4
};
// 内存布局：[c][填充3字节][i i i i]
...
// sizeof(Optimized) = 8，但布局更合理
```

== 类对象大小计算原理

*1. 成员变量占用空间*
- 普通成员变量：累加所有成员的大小（含对齐填充）
- 静态成员：不占对象空间
- 成员函数：不占对象空间
...
class Example {
    int a;              // 4字节
    static int s;       // 0字节（不占对象空间）
    void func() {}      // 0字节（不占对象空间）
};
sizeof(Example);  // 4字节
```

*2. 虚函数的影响*
- 有虚函数的类会增加一个虚函数表指针（vptr）
- vptr 大小：32位系统4字节，64位系统8字节
- 无论有多少个虚函数，只有一个 vptr
...
sizeof(NoVirtual);  // 4字节

class HasVirtual {
    int a;
    virtual void foo() {}
    virtual void bar() {}  // 多个虚函数也只有一个 vptr
};
sizeof(HasVirtual);  // 16字节（64位）= 8（vptr）+ 4（int）+ 4（对齐填充）
```

*3. 空类特殊规则*
```cpp
class Empty {};
sizeof(Empty);  // 1字节（保证不同对象有不同地址）
```
...
int arr[10];
void func(int arr[]) {
    sizeof(arr);  // 8（指针大小），不是40！
}
sizeof(arr);  // 40（在 main 中是数组）

// 动态分配的数组
int* p = new int[10];
sizeof(p);  // 8（指针大小），不能获取数组大小
```

== 总结

*sizeof 的三个核心原理*：
1. *编译期计算*：不会执行表达式，结果在编译时确定
2. *内存对齐*：结构体/类的实际大小包含填充字节，遵循对齐规则
3. *虚函数表指针*：有虚函数的类会增加 vptr（8字节/64位）
...
在 C++11 之前，模板只能接受固定数量的参数，导致代码冗余：
...
print(1);                    // 1个参数
print(1, "hello");           // 2个参数
print(1, "hello", 3.14);     // 3个参数
```

*核心语法*：

```cpp
template<typename... Args>  // Args 是类型参数包（声明）
//               ^^^
//            声明参数包
...
void func(Args&&... args);     // 展开 Args，声明多个参数

// 2. 在标识符后：展开参数包
helper(args...);               // 展开为：arg1, arg2, arg3

// 3. 在模式后：模式展开
helper(process(args)...);      // 展开为：process(arg1), process(arg2), ...
```
...
void example(Args... args) {
    // 假设调用：example(1, "hello", 3.14)
    // Args = {int, const char*, double}
    // args 代表 {1, "hello", 3.14}
    
    // 1. 简单展开
    helper(args...);
    // 展开为：helper(1, "hello", 3.14);
    
    // 2. 模式展开
    helper(process(args)...);
    // 展开为：helper(process(1), process("hello"), process(3.14));
    ...
    // 4. 获取参数个数
    constexpr size_t count = sizeof...(Args);  // count = 3
    constexpr size_t count2 = sizeof...(args); // count2 = 3
}
```
...
// 展开过程：
// print(1, "hello", 3.14, 'A')
//   -> 输出 1，调用 print("hello", 3.14, 'A')
//     -> 输出 hello，调用 print(3.14, 'A')
//       -> 输出 3.14，调用 print('A')
//         -> 输出 A，调用 print()
//           -> 输出换行，结束
```
...
auto vec = make_unique<std::vector<int>>(5, 100);  // 创建5个100的vector
```
...
std::vector<std::pair<int, std::string>> vec;
vec.emplace_back(1, "hello");  // 原地构造 pair，零拷贝
```
...
auto measure_time(Func f, Args&&... args) {
    ...
    auto result = f(std::forward<Args>(args)...);  // 完美转发
...
// 使用
int result = measure_time(compute, 100, "data");  // 测量 compute(100, "data") 的时间
```
...
safe_print(1, "hello", 3.14, 'A');  // 输出：1 hello 3.14 A
...
template<typename T, typename... Args>
void safe_print(const T& first, const Args&... rest) {
    std::cout << first << " ";
    safe_print(rest...);
}
```
...
void parallel_execute(Funcs&&... funcs) {
    std::vector<std::thread> threads;
    
    // C++17 折叠表达式
    (threads.emplace_back(std::forward<Funcs>(funcs)), ...);
...
// 创建 tuple
template<typename... Args>
auto make_tuple_example(Args&&... args) {
    return std::make_tuple(std::forward<Args>(args)...);
}

auto t = make_tuple_example(1, "hello", 3.14);
...
auto apply(Func f, Tuple&& t) {
    ...
    return apply_impl(f, std::forward<Tuple>(t), std::make_index_sequence<N>{});
}

// 使用
int add3(int a, int b, int c) { return a + b + c; }
auto tuple = std::make_tuple(1, 2, 3);
int result = apply(add3, tuple);  // result = 6
```
...
// 检查所有类型是否相同
template<typename T, typename... Args>
constexpr bool all_same_type = (std::is_same_v<T, Args> && ...);

static_assert(all_same_type<int, int, int>, "All should be int");
// static_assert(all_same_type<int, int, double>, "");  // 失败

// 检查所有类型是否可移动
template<typename... Args>
constexpr bool all_movable = (std::is_move_constructible_v<Args> && ...);
```
...
print_integers(1, "skip", 2, 3.14, 3);  // 输出：1 2 3
```

*注意事项*：

```cpp
// 1. 参数包必须是最后一个模板参数（除非后面的参数有默认值）
template<typename... Args, typename T>  // ✗ 错误
void bad_func(Args... args, T t);

template<typename T, typename... Args>  // ✓ 正确
void good_func(T t, Args... args);
...
template<typename T>
struct Wrapper<T, T> {};  // ✗ 错误

// 3. 空参数包是合法的
template<typename... Args>
void func(Args... args) {}

func();  // ✓ 合法，Args 和 args 都为空
...
// 编译后等价于：
void process_compiled(int&& arg1, const char*&& arg2, double&& arg3) {
    helper(std::forward<int>(arg1), 
           std::forward<const char*>(arg2),
           std::forward<double>(arg3));
}
...
可变参数模板的核心概念：
1. *参数包*：使用 `...` 声明和展开
2. *模式展开*：`pattern...` 对每个元素应用模式
3. *递归*：C++11/14 主要方式
4. *折叠表达式*：C++17 更简洁的方式
5. *完美转发*：与 `std::forward` 结合，保持参数属性
6. *编译期展开*：零运行时开销
